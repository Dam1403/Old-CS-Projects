...
public class MST{
	...
	public static void printPrim(ArrayList<int[]> alledges){
		ArrayList<int[]> mstData = Prim.doPrimStuff(alledges,n);
		ArrayList<int[]> sorted = new ArrayList<int[]>();
		int weight = 0;
		for(int i = 0; i < mstData.size();i++){
			weight += mstData.get(i)[2];
		}
		for(int i = 0; i < mstData.size();i++){
			for(int j = 0; j < mstData.size();j++){
				if(i + 1  == mstData.get(j)[1]){
					sorted.add(mstData.get(j));
				}
			}
		}
		printSortedList(sorted);
		System.out.println("\n" + "Total weight of MST using Prim: " + weight);
	}
	public static void printPrim(Edges alledges){
		ArrayList<int[]> mstData = Prim.doPrimStuff(alledges.getAllEdges(),n);
		ArrayList<int[]> sorted = new ArrayList<int[]>();
		int weight = 0;
		for(int i = 0; i < mstData.size();i++){
			weight += mstData.get(i)[2];
